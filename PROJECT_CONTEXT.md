...
**Status:** Day 6 (Stage 4.3 shipped — idempotent demo patient seed + evaluation docs; Stage 4.1–4.2 AI protection done; Stage 5.1 longitudinal insights next)
...
**Env:** `GEMINI_API_KEY` required for interpret + prescription Vision + document entity lane; optional `GEMINI_VISION_MODEL` pins the Vision model and optional `GEMINI_TEXT_MODEL` pins the text entity model (both default `gemini-flash-latest`; set e.g. `gemini-2.5-flash` if the alias returns 503); `JWT_SECRET` required for auth (documented in `.env.example`).
